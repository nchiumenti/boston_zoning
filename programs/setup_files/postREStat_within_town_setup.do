...
noisily display "Running postQJE_within_town_setup.do..."
...
/* Start of REStat Revisions from 03-27-2024
	- add another distance variable for relaxed3 */
...
// Eli : this assertion is coming back as false even though it's very close
// r(mean) is  7.547702495636593
sum log_combrent1
	noisily assert `r(N)' ==  3167190
	noisily assert `r(sum_w)' ==  3167190
	//noisily assert `r(mean)' ==  7.547702495636594
	noisily assert `r(mean)' ==  7.547702495636593 // new from Eli
...
sum log_combrent2
	...
	// eli: this is also coming back false
	//  r(mean) is 680570.7492073558
sum def_saleprice
	noisily assert `r(N)' ==  568782
	noisily assert `r(sum_w)' ==  568782
	//noisily assert `r(mean)' ==  680570.7492073559
	noisily assert `r(mean)' ==  680570.7492073558
