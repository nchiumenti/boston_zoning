...
*					Zoning Regulations
*
* Description:		Creates the A.3 scatter plot of number of boundaries 
*					per acre relative to various soc-econ measures.
*
*					The goal of this file is to support there being no 
*					correlation between number of zoning boundaries and various
...
confirm file "$DATAPATH/muni_tax_rates.dta"
confirm file "$DATAPATH/muni_land_area.dta"

scalar FIX_YEAR_FORMAT = 0
if FIX_YEAR_FORMAT {

	use "$DATAPATH/muni_tax_rates.dta",clear
	destring year,gen(yr)
	drop year 
	gen double year = yr 
	drop yr
	order year cousub_name res_rate
	save  "$DATAPATH/muni_tax_rates.dta",replace
}
********************************************************************************
...
foreach l in m_units m_rent m_price m_frac_under18 m_frac_over65 m_frac_mortgage m_frac_rented m_frac_female m_frac_black m_frac_asian m_frac_hispanic m_frac_nonhispanicwhite m_frac_morethan4 m_median_inc m_total_pop m_SHARE_CAR_MBIKE m_SHARE_PUBLICTRANS m_SHARE_INC_OVER200K m_SHARE_BACHELOR_25 m_natwalkind m_res_rate m_boundaies_per_acre m_boundaries_per_sqmi{
	...
	* [PAPER SOURCE] for Figure A.3->
	*scatter against number of boundaries per area
	twoway scatter `l' m_boundaies_per_acre, xtitle("# of boundaries per acre in city") ytitle("`l'") name("`l'",replace)
	graph save "`l'" "`l'b.gph", replace
