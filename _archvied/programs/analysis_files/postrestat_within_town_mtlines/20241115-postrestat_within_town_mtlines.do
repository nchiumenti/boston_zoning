...
forvalues i=1/5{ 
	...
	* A: only_mf
	quietly reg log_mfrent i.mf_allowed i.lam_seg `distance_varlist`i'' i.year ///
		if (only_mf == 1 & dist <= 0.02 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_mfrent height i.lam_seg `distance_varlist`i'' i.year ///
		if (only_he == 1 & dist <= 0.02 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_mfrent dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (only_du == 1 & dist <= 0.02 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_mfrent i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he == 1 & dist <= 0.02 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_mfrent i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_du == 1 & dist <= 0.02 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_mfrent c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (du_he == 1 & dist <= 0.02 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_mfrent i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he_du == 1 & dist <= 0.02 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
...
forvalues i=1/5{
	* A: only_mf
	quietly reg log_mfrent i.mf_allowed i.lam_seg `distance_varlist`i'' i.year ///
		if (only_mf == 1 & dist <= 0.05 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_mfrent height i.lam_seg `distance_varlist`i'' i.year ///
		if (only_he == 1 & dist <= 0.05 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_mfrent dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (only_du == 1 & dist <= 0.05 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_mfrent i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he == 1 & dist <= 0.05 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_mfrent i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_du == 1 & dist <= 0.05 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_mfrent c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (du_he == 1 & dist <= 0.05 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_mfrent i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he_du == 1 & dist <= 0.05 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
...
forvalues i = 1/5{
	* A: only_mf
	quietly reg log_mfrent i.mf_allowed i.lam_seg `distance_varlist`i'' i.year ///
		if (only_mf == 1 & dist <= 0.10 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_mfrent height i.lam_seg `distance_varlist`i'' i.year ///
		if (only_he == 1 & dist <= 0.10 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_mfrent dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (only_du == 1 & dist <= 0.10 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_mfrent i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he == 1 & dist <= 0.10 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_mfrent i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_du == 1 & dist <= 0.10 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_mfrent c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (du_he == 1 & dist <= 0.10 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_mfrent i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he_du == 1 & dist <= 0.10 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
...
forvalues i = 1/5{
	* A: only_mf
	quietly reg log_mfrent i.mf_allowed i.lam_seg `distance_varlist`i'' i.year ///
		if (only_mf == 1 & dist <= 0.15 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_mfrent height i.lam_seg `distance_varlist`i'' i.year ///
		if (only_he == 1 & dist <= 0.15 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_mfrent dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (only_du == 1 & dist <= 0.15 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_mfrent i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he == 1 & dist <= 0.15 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_mfrent i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_du == 1 & dist <= 0.15 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_mfrent c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (du_he == 1 & dist <= 0.15 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_mfrent i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he_du == 1 & dist <= 0.15 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
...
forvalues i =1/5{
	* A: only_mf
	quietly reg log_mfrent i.mf_allowed i.lam_seg `distance_varlist`i'' i.year ///
		if (only_mf == 1 & dist <= 0.20 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_mfrent height i.lam_seg `distance_varlist`i'' i.year ///
		if (only_he == 1 & dist <= 0.20 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_mfrent dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (only_du == 1 & dist <= 0.20 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_mfrent i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he == 1 & dist <= 0.20 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_mfrent i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_du == 1 & dist <= 0.20 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_mfrent c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (du_he == 1 & dist <= 0.20 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_mfrent i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.year ///
		if (mf_he_du == 1 & dist <= 0.20 & res_typex != "Condominiums" & (year >= 2010 & year <= 2018)), vce(cluster lam_seg)
...
forvalues i = 1/5{
	* A: only_mf
	quietly reg log_saleprice i.mf_allowed i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_mf == 1 & dist <= 0.02 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_saleprice height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_he == 1 & dist <= 0.02 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_saleprice dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_du == 1 & dist <= 0.02 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_saleprice i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he == 1 & dist <= 0.02 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_saleprice i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_du == 1 & dist <= 0.02 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_saleprice c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (du_he == 1 & dist <= 0.02 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_saleprice i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he_du == 1 & dist <= 0.02 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
...
forvalues i = 1/5{
	* A: only_mf
	quietly reg log_saleprice i.mf_allowed i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_mf == 1 & dist <= 0.05 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_saleprice height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_he == 1 & dist <= 0.05 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_saleprice dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_du == 1 & dist <= 0.05 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_saleprice i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he == 1 & dist <= 0.05 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_saleprice i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_du == 1 & dist <= 0.05 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_saleprice c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (du_he == 1 & dist <= 0.05 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_saleprice i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he_du == 1 & dist <= 0.05 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
...
forvalues i = 1/5{
	* A: only_mf
	quietly reg log_saleprice i.mf_allowed i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_mf == 1 & dist <= 0.10 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_saleprice height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_he == 1 & dist <= 0.10 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_saleprice dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_du == 1 & dist <= 0.10 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_saleprice i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he == 1 & dist <= 0.10 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_saleprice i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_du == 1 & dist <= 0.10 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_saleprice c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (du_he == 1 & dist <= 0.10 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_saleprice i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he_du == 1 & dist <= 0.10 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
...
forvalues i = 1/5{
	* A: only_mf
	quietly reg log_saleprice i.mf_allowed i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_mf == 1 & dist <= 0.15 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo A

	* B: only_he
	quietly reg log_saleprice height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_he == 1 & dist <= 0.15 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
		
		eststo B

	* C: only_du
	quietly reg log_saleprice dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (only_du == 1 & dist <= 0.15 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 
		
		eststo C

	* D: mf_he
	quietly reg log_saleprice i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he == 1 & dist <= 0.15 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* E: mf_du
	quietly reg log_saleprice i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_du == 1 & dist <= 0.15 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg) 

		eststo E

	* F: du_he
	quietly reg log_saleprice c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (du_he == 1 & dist <= 0.15 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_saleprice i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he_du == 1 & dist <= 0.15 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
...
forvalues i = 1/5{
	...
	* G: all (MF, Height and DUPAC)
	quietly reg log_saleprice i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' i.last_saleyr ///
		if (mf_he_du == 1 & dist <= 0.20 & res_typex == "Single Family Res" & (last_saleyr >= 2010 & last_saleyr <= 2018)), vce(cluster lam_seg)
...
* 5c: number of units baseline after 1918 @ .20 miles w/ only clear boundaries
********************************************************************************
...
forvalues i = 1/5 {
    * A: only_mf
//     quietly reg num_units1 i.mf_allowed i.lam_seg `distance_varlist`i'' ///
//         if only_mf == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)
        
			quietly reg num_units1 i.mf_allowed i.lam_seg `distance_varlist1' ///
			if only_mf == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)
    ...
    * B: only_he
    quietly reg num_units1 height i.lam_seg `distance_varlist`i'' ///
        if only_he == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)

        eststo B_new

    * C: only_du
    quietly reg num_units1 dupac i.lam_seg `distance_varlist`i'' ///
        if only_du == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)

        eststo C_new

    * D: mf_he
    quietly reg num_units1 i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' ///
        if mf_he == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)
    ...
    * E: mf_du
    quietly reg num_units1 i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' ///
        if mf_du == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)
    ...
    * G: du_he
    quietly reg num_units1 c.height##c.dupac i.lam_seg `distance_varlist`i'' ///
        if du_he == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)
    ...
    * H: all (mf du he)
		*Note: no observations error 20240920 -- H_new removed
//     quietly reg num_units1 i.mf_allowed##c.height##c.dupac i.lam_seg `distance_varlist`i'' ///
//         if mf_he_du == 1 & clear_relaxed_strict_lam == 1 & dist <= 0.2 & year_built >= 1918 & year == 2018, vce(cluster lam_seg)
//        
//         eststo H_new
//        
//         test 1.mf_allowed height dupac
...
//         test 1.mf_allowed 1.mf_allowed#c.dupac 1.mf_allowed#c.height 1.mf_allowed#c.height#c.dupac
    ...
    * combine results
    esttab A_new B_new C_new D_new E_new F_new G_new, se r2 nobase indicate("Boundary f.e.=*lam_seg") ///
        label mtitles("only_mf" "only_he" "only_du" "mf_he" "mf_du" "du_he") ///
        title("Part 5c: number of units baseline after 1918 @ .20 miles (distance polynomial trends degree `i'), only clear boundaries") 
...
forvalues i = 1/5 {
    * A: only_mf
    quietly reg fam23_1 i.mf_allowed i.lam_seg `distance_varlist`i'' ///
        ...
        eststo A_new
    *POSTRESTAT
    sum fam23_1 if only_mf == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam23_1 if only_mf == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    * B: only_he
    quietly reg fam23_1 height i.lam_seg `distance_varlist`i'' ///
        ...
        eststo B_new
    *POSTRESTAT
    sum fam23_1 if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam23_1 if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum height if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum height if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    * C: only_du
    quietly reg fam23_1 dupac i.lam_seg `distance_varlist`i'' ///
        ...
        eststo C_new
    *POSTRESTAT
    sum fam23_1 if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam23_1 if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum dupac if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum dupac if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    * D: mf_he
    quietly reg fam23_1 i.mf_allowed##c.height i.lam_seg `distance_varlist`i'' ///
        ...
        * mf_allowed
        test 1.mf_allowed 1.mf_allowed#c.height
    *POSTRESTAT
    sum fam23_1 if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam23_1 if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum height if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum height if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    * E: mf_du
    quietly reg fam23_1 i.mf_allowed##c.dupac i.lam_seg `distance_varlist`i'' ///
        ...
        eststo E_new
    *POSTRESTAT
    sum fam23_1 if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam23_1 if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum dupac if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum dupac if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    * G: du_he
    quietly reg fam23_1 c.height##c.dupac i.lam_seg `distance_varlist`i'' ///
        ...
        * height
        test height c.height#c.dupac
    *POSTRESTAT
    sum fam23_1 if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam23_1 if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum dupac if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum dupac if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum height if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum height if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
...
//         test 1.mf_allowed 1.mf_allowed#c.dupac 1.mf_allowed#c.height 1.mf_allowed#c.height#c.dupac
    *POSTRESTAT
    sum fam23_1 if mf_he_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam23_1 if mf_he_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"

    * combine results
    esttab A_new B_new C_new D_new E_new F_new G_new, se r2 nobase indicate("Boundary f.e.=*lam_seg") ///
        label mtitles("only_mf" "only_he" "only_du" "mf_he" "mf_du" "du_he") ///
        title("Part 6c: gentle density baseline after 1918 @ .20 miles (distance polynomial trends degree `i'), only clear boundaries") 
...
forvalues i = 1/5 {
    ...
    *POSTRESTAT
    sum fam4plus_1 if only_mf == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam4plus_1 if only_mf == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    *POSTRESTAT
    sum fam4plus_1 if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam4plus_1 if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum height if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum height if only_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    *POSTRESTAT
    sum fam4plus_1 if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam4plus_1 if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum dupac if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum dupac if only_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    *POSTRESTAT
    sum fam4plus_1 if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam4plus_1 if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum height if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum height if mf_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    *POSTRESTAT
    sum fam4plus_1 if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam4plus_1 if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum dupac if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum dupac if mf_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    ...
    *POSTRESTAT
    sum fam4plus_1 if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam4plus_1 if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum dupac if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum dupac if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
    sum height if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum height if du_he == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"
...
//         test 1.mf_allowed 1.mf_allowed#c.dupac 1.mf_allowed#c.height 1.mf_allowed#c.height#c.dupac
    ...
    *POSTRESTAT
    sum fam4plus_1 if mf_he_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0.02 & dist_both > 0) & res_typex != "Condominiums"
    sum fam4plus_1 if mf_he_du == 1 & clear_relaxed_strict_lam == 1 & year == 2018 & year_built >= 1918 & (dist_both <= 0 & dist_both > -0.02) & res_typex != "Condominiums"

    * combine results
    esttab A_clear B_clear C_clear D_clear E_clear F_clear G_clear, se r2 nobase indicate("Boundary f.e.=*lam_seg") ///
        label mtitles("only_mf" "only_he" "only_du" "mf_he" "mf_du" "du_he") ///
        title("Part 7c: high density baseline after 1918 @ .20 miles (distance polynomial trends degree `i'), only clear boundaries") 
